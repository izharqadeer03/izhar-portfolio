...
function StartPanel({ onClose, triggerRef }: Omit<WindowsStartMenuProps, 'isOpen'>) {
  ...
  return (
    <>
      {/* Subtle backdrop overlay that dims the desktop slightly and dismisses on outside click */}
      <motion.div
        className="fixed inset-0 z-155 bg-black/35 backdrop-blur-[2px]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="fixed left-1/2 z-160 flex w-[min(560px,94vw)] -translate-x-1/2 flex-col overflow-hidden rounded-2xl border border-white/12 bg-[#0f141d]/96 backdrop-blur-3xl"
        style={{
          bottom: TASKBAR_HEIGHT + 10,
          boxShadow:
            '0 40px 90px -24px rgba(0,0,0,0.95), inset 0 1px 0 rgba(255,255,255,0.12), inset 0 0 0 1px rgba(255,255,255,0.04)',
      ...
      >
        ...
        <div className="px-5 pt-5 pb-4">
          <div className="relative">
            ...
            <input
              ...
              className={cn(
                'h-10 w-full rounded-full border border-line-strong bg-void/60 ps-10 pe-4 text-[13px] text-fg',
                'placeholder:text-faint focus:outline-none',
                'focus:border-[color:color-mix(in_oklab,var(--env-accent)_65%,transparent)] focus:ring-1 focus:ring-[color:color-mix(in_oklab,var(--env-accent)_40%,transparent)]',
                  ...
                  <li key={result.id} role="none">
                    <button
                      ...
                      className={cn(
                        ...
                        'transition-colors duration-150 ease-env focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/70',
                        index === launcher.highlight
                          ? 'border-line-strong bg-white/[0.09]'
                ...
                <li key={application.id} role="none">
                  <button
                    ...
                    className={cn(
                      ...
                      'transition-all duration-150 ease-env focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/70',
                      index === launcher.highlight
                        ? 'border-line-strong bg-white/[0.09] shadow-sm'
        ...
        {/* Account row. */}
        <footer className="flex items-center gap-3 border-t border-line bg-void/50 px-5 py-3">
